...
public class fragment_class_total_topics extends Fragment {
    // firestore
    CollectionReference analytics;
    // widgets
    public BarChart barChartTopics;
    // vars
    public int total_topics = 0;

    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View v = inflater.inflate(R.layout.fragment_analytics_class_total_topics, container, false);
    ...
    public void initGraphs() {
        ...
        analytics = FirebaseFirestore.getInstance().collection("Classes").document(classroom_id).collection("Posts");
        analytics
                .get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            total_topics = task.getResult().size();
                        }

                        // init bar graph with queried values.
                        List<BarEntry> entriesUsers = new ArrayList<>();
                        entriesUsers.add(new BarEntry(0f, total_topics));
                        BarDataSet barSet = new BarDataSet(entriesUsers, "Total topics");
